...
### support Molcas, G16, BAGEL, ORCA Aug 19 2020 Jingbai Li
...
def Boltzmann(sample):
    ...
    ## q = sqrt(kb*T/m)/w = sqrt(T/(Eh/kb))/[ sqrt(m/me) * w*h_bar]*sqrt(Eh*h_bar^2/me) = sqrt(T/(Eh/kb))/[ sqrt(m/me)*mu*(h*c/Eh) ]*sqrt(h_bar^2/me*Eh) = sqrt(T/(Eh/kb))/[ sqrt(m/me)*mu*(h*c/Eh) ]*a0 
    ## q = sqrt(T/(Eh/kb))/[ sqrt(m/me)*mu*(h*c/Eh) ]in [Bohr]
    ##
    ## Convert velocity[m/s] to atomic unit[Bohr/au]
    ## v = sqrt(kb*T/m) = sqrt(T/(Eh/kb)/sqrt(m/me))*sqrt(Eh/me) = sqrt(T/(Eh/kb)/sqrt(m/me))*vau
    ## v = sqrt(T/(Eh/kb)/sqrt(m/me)) in [Bohr/au]
...
def Sampling(input,nesmb,iseed,temp,dist,format):
    ...
    if format in callsample:
        ...
        if   len(ensemble) < nesmb:
            print('Not enough initial conditions!!! %s provided < %s requested.' % (len(ensemble),nesmb))
            exit()
    ...
    for mol in ensemble:
        ...
        velo=mol[:,4:7]
        natom=len(geom)
        np.savetxt(q, geom,header='%s\n [Angstrom]' % (len(geom)),comments='',fmt='%-5s%30s%30s%30s')
        np.savetxt(p, velo,header='%d [Bohr / time_au]' % (m),comments='',fmt='%30s%30s%30s')
        np.savetxt(pq,mol,header='Init %5d %5s %12s%30s%30s%30s%30s%30s%22s%6s' % (m,natom,'X(A)','Y(A)','Z(A)','VX(au)','VY(au)','VZ(au)','g/mol','e'),comments='',fmt='%-5s%30s%30s%30s%30s%30s%30s%16s%6s')
    q.close()
    p.close()
    pq.close()
